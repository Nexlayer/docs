...
import { FileText, Code, Database, Globe, Settings, ExternalLink, Play } from "lucide-react";
...
export default function TemplatesPage() {
  const templates = [
    ...
    },
    {
      ...
      features: ["Custom Stack", "Cloudbeaver", "Database Management", "Fullstack"],
      complexity: "Advanced",
      color: "bg-orange-500"
    },
    {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16 relative z-10">
          ...
          <div className="max-w-4xl mx-auto mb-12">
            <div className="bg-[#0a0a0a] rounded-xl p-8 border border-[#333]">
              <h2 className="text-2xl font-bold text-white mb-4">Ready-to-Use Templates</h2>
              <p className="text-gray-300 mb-6">
                ...
                . Each template includes a complete YAML configuration that you can deploy instantly 
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {templates.map((template, index) => (
              <div key={index} className="bg-[#0a0a0a] rounded-xl p-6 border border-[#333] hover:border-[#22B4C8] transition-colors">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {template.icon}
                    <h3 className="text-xl font-bold text-white">{template.title}</h3>
                  </div>
                  <div className={`${template.color} text-white text-xs px-2 py-1 rounded-full font-medium`}>
                    {template.complexity}
                  </div>
                </div>
                ...
                <p className="text-gray-300 mb-4">{template.description}</p>
                         ...
                         <div className="flex flex-wrap gap-2">
                           {template.features.map((feature, featureIndex) => (
                             <span key={featureIndex} className="text-[#22B4C8] text-sm">
                               {feature}{featureIndex < template.features.length - 1 ? ' •' : ''}
                             </span>
                           ))}
                         </div>
                       </div>
                ...
                <div className="flex gap-2">
                  <Link href={template.path}>
                    <Button className="bg-[#22B4C8] hover:bg-[#1DA3B6] text-black px-4 py-2 rounded-lg text-sm">
                  ...
                  <a 
                    ...
                    rel="noopener noreferrer"
                  >
                    <Button variant="outline" className="border-[#333] text-gray-300 hover:bg-[#111] px-4 py-2 rounded-lg text-sm">
          ...
          <div className="text-center">
            <div className="bg-[#0a0a0a] rounded-xl p-10 border border-[#333] max-w-2xl mx-auto">
              ...
              <p className="text-gray-300 mb-8">
                Try these templates in the Nexlayer Playground for instant deployment.
              </p>
